...
export function AdminNavigation({
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-cream">
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-50 flex flex-col w-[240px] bg-[#001D4E] border-r border-white/10
          transition-transform duration-300 md:hidden
          ${mobileOpen ? "translate-x-0" : "-translate-x-full"}
        `}
      >
        {/* Cabecera Sidebar Mobile */}
        <div className="flex items-center justify-between px-4 py-5 border-b border-white/10 min-h-[65px]">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-sans font-bold text-sm text-white">SendHope Admin</span>
          </div>
          <button
            onClick={() => setMobileOpen(false)}
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <header className="flex-shrink-0 flex items-center justify-between bg-[#001D4E] text-white px-5 py-3 border-b border-white/10 h-[65px]">
          {/* Hamburger button + logo on mobile */}
          <div className="flex items-center gap-4">
            <button
              onClick={() => setMobileOpen(true)}
              className="flex flex-col justify-center gap-1.5 w-6 h-6 md:hidden text-white/80 hover:text-white"
              aria-label="Abrir menú de navegación"
            >
              <span className="block w-6 h-[2.5px] bg-white rounded-full transition-transform" />
              <span className="block w-6 h-[2.5px] bg-white rounded-full transition-transform" />
              <span className="block w-6 h-[2.5px] bg-white rounded-full transition-transform" />
            </button>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-sans font-bold text-sm block md:hidden">SendHope Admin</span>
            </div>
          </div>
